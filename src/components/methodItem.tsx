...
export default function MethodItem({ icon, title, selected, onSelect, discount }: MethodItemProps) {
    return (
        <button
            className={twMerge([
                "flex-1 min-h-28 text-left bg-secondary-background border-border-background border rounded-xl p-4 box-border",
        ...
        >
            {
                icon === "Pix" ? (
                    ...
                    :
                    <Lucide icon={icon} className=" w-6 h-6 stroke-2" />
            }
